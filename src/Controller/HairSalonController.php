...
/**
 * @Route("/hairsalon", name="hairsalon_")
 */
class HairSalonController extends AbstractController
{
    ...
    public function index()
    {
        ...
        $service = $this->serviceRepository->findOneby(['id' => 3]);
    ...
    function product($id)
    {
        ...
        $allproducts = array();
        $company = $this->mooveeCompanyRepository->findOneBy(['code_entreprise' => $user->getCodeCompany()]);
        $availabilities = $this->availabilityRepository->findIncomingByServiceNoNeedCar($company, $id, 3);
        ...
        return $this->render('hair_salon/product.html.twig', [
    ...
    function options(Request $request, $serviceid, $id)
    {
        ...
        if ($categoryOptions != null) {
            return $this->render('hair_salon/option.html.twig', [
            ...
            return $this->render('hair_salon/locationchoice.html.twig', [
                'duration' => $currentproduct->getTime(),
    ...
    public function chooseLocation(Request $request, $id)
    {
        ...
        try {
            ...
            return $this->redirectToRoute('hairsalon_hair_salon');
        }
    }
    /**
    ...
    public function chooseDate(Request $request)
    {
        ...
        try {
            ...
            return $this->render('hair_salon/datechoice.html.twig', [
                'duration' => $categoryProduct->getTime(),
                'achat' => $categoryProduct->getIdProduct()->getDirectBuy(),
                'custom_layout' => $this->mooveeCompanyRepository->findOneBy(['code_entreprise' => $user->getCodeCompany()]),
                'categoryproduct' => $categoryProduct,
    ...
    public function mycalendar()
    {
        ...
        try {
            ...
            return $this->render('hair_salon/mycalendar.html.twig', [
                'product' => $product,
                'duration' => $categoryProduct->getTime(),
                'availabilities' => $availabilities,
                'custom_layout' => $this->mooveeCompanyRepository->findOneBy(['code_entreprise' => $user->getCodeCompany()]),
    ...
    public function resumeDirect(Request $request)
    {
        ...
        try {
            ...
            return $this->render('hair_salon/resume.html.twig', [
                'categoryProduct' => $categoryProduct,
                'options' => $options,
    ...
    public function paiment(Request $request)
    {
        if ($request->getMethod() == 'POST') {
            ...
            if ($price == 0) {
                ...
                if ($decimal == 0)
                    $decimal = 100;
                return $this->render('hair_salon/payment.html.twig', [
                    'user' => $user,
    ...
    public function payWithSavedCard(Request $request, int $price, int $decimal)
    {
        if ($request->getMethod() == 'POST') {
            ...
            if ($product->getIdProduct()->getDirectBuy() == true || $product->getIdProduct()->getAutoValide() == true) {
                ...
                //toUser
                $message->sendReservation($plannedcleaning->getUserId()->getEmail(), $facture, $dateOfService, $plannedcleaning->getProduit()->getOption(), $plannedcleaning->getProduit()->getName(),  number_format($price, 2, ' . ', ', '), $icsDocument);
                //toProvider
                foreach ($provider as $p) {
                    $messagePro->sendMailDirectToProvider($p->getEmail(), $mvuser, $dateOfService, $availability->getParking()->getName(), $plannedcleaning->getProduit()->getName(), $plannedcleaning->getProduit()->getOption(), $icsDocumentProvider);
                }
                ...
                $message->sendReservationIndirect($plannedcleaning->getUserId()->getEmail(), $dateOfService, $plannedcleaning->getProduit()->getOption(), $plannedcleaning->getProduit()->getName(),  number_format($price, 2, ' . ', ', '), $icsDocument);
    ...
    public function payWithNewCard(Request $request, int $price, int $decimal)
    {
        if ($request->getMethod() == 'POST') {
            ...
            $sEvent = new DateTime($startEvent);
            $eEvent = new DateTime($endEvent);
            $icsDocument = $this->iscFile(
                $sEvent->format('Ymd\THis'),
                $eEvent->format('Ymd\THis'),
                $product->getIdProduct()->getDescription(),
                $product->getIdProduct()->getName(),
                $availability->getParking()->getAddress()
            );
    ...
    public function valide(Request $request, int $id)
    {
        ...
        return $this->redirectToRoute("hair_salon_dashboard");
    }
